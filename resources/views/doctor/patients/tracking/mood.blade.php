...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div>
        ...
        <h1 class="h3 mb-1 fw-semibold">Mood Logs</h1>
        <p class="text-muted mb-0">Mood tracking entries for {{ $patient->name ?? $patient->full_name ?? optional($patient->user)->name ?? 'Patient' }}</p>
    </div>
</div>
...
<div class="card border-0 shadow-sm">
    <div class="card-body p-0">
        ...
        <div class="text-center py-5">
            ...
            <h5 class="mt-3 mb-2">No mood logs</h5>
            <p class="text-muted mb-0">The client has not logged any mood entries yet.</p>
        </div>
        @else
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $log->log_time ? \Carbon\Carbon::parse($log->log_time)->format('H:i') : '—' }}</td>
                        <td>{{ $log->mood_rating ?? '—' }}{!! isset($log->mood_emoji) ? ' ' . $log->mood_emoji : '' !!}</td>
        ...
        <div class="p-3">
            {{ $moodLogs->withQueryString()->links() }}
        </div>
        @endif
    </div>
</div>
