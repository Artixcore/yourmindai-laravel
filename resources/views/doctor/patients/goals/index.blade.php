...
<div class="container-fluid">
        ...
        <div class="col-12">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0">Set and track goals for {{ $patient->user->name ?? $patient->full_name ?? 'Patient' }}</p>
                </div>
                <a href="{{ route('patients.goals.create', $patient->id) }}" class="btn btn-primary"><i class="bi bi-plus-lg me-2"></i>New Goal</a>
            </div>
        </div>
    </div>
    @if(session('success'))
    ...
    @endif
    @if($goals->isEmpty())
        <div class="card shadow-sm"><div class="card-body text-center py-5"><i class="bi bi-bullseye text-muted" style="font-size: 3rem;"></i><p class="text-muted mt-3 mb-0">No goals set yet.</p><a href="{{ route('patients.goals.create', $patient->id) }}" class="btn btn-primary mt-3">Create first goal</a></div></div>
    @else
        <div class="card shadow-sm"><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover mb-0"><thead class="table-light"><tr><th>Title</th><th>Start / End</th><th>Frequency</th><th>Duration</th><th>Status</th><th>Visibility</th><th></th></tr></thead><tbody>
            ...
            <tr>
                ...
                <td><a href="{{ route('patients.goals.edit', [$patient->id, $goal]) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                    <form action="{{ route('patients.goals.destroy', [$patient->id, $goal]) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this goal?');">@csrf @method('DELETE')<button type="submit" class="btn btn-sm btn-outline-danger">Delete</button></form></td>
            </tr>
            @endforeach
