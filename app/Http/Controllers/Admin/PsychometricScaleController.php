...
class PsychometricScaleController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = [
            'total' => PsychometricScale::count(),
            'active' => PsychometricScale::where('is_active', true)->count(),
            'inactive' => PsychometricScale::where('is_active', false)->count(),
            'by_category' => PsychometricScale::select('category', DB::raw('COUNT(*) as count'))
                ->groupBy('category')
                ->pluck('count', 'category')
                ->toArray(),
        ];

        return view('admin.psychometric-scales.index', compact('scales', 'stats'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.psychometric-scales.index')
    ...
    public function show(PsychometricScale $psychometricScale)
    {
        ...
        $usageStats = [
            'total_assessments' => $psychometricScale->assessments()->count(),
            'completed_assessments' => $psychometricScale->assessments()->where('status', 'completed')->count(),
            'pending_assessments' => $psychometricScale->assessments()->where('status', 'pending')->count(),
            'avg_score' => round($psychometricScale->assessments()->where('status', 'completed')->avg('total_score'), 2),
        ];
        
        return view('admin.psychometric-scales.show', compact('psychometricScale', 'usageStats'));
    }
    ...
    {
        $this->authorize('delete', $psychometricScale);
    ...
    /**
     * Toggle the active status of a scale.
     */
    public function toggleActive(PsychometricScale $psychometricScale)
    {
        $psychometricScale->update([
            'is_active' => !$psychometricScale->is_active
        ]);

        $status = $psychometricScale->is_active ? 'activated' : 'deactivated';

        return redirect()->back()
            ->with('success', "Psychometric scale has been {$status} successfully.");
    }
}
