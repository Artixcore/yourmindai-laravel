...
<div class="container mx-auto px-4 py-6 pb-24">
    <div class="mb-6">
        <a href="{{ route('client.reviews.show', $review) }}" class="inline-flex items-center text-purple-600 hover:text-purple-700 mb-4">
    ...
    <form action="{{ route('client.reviews.update', $review) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <x-star-rating name="overall_rating" :rating="old('overall_rating', $review->overall_rating)" size="lg" />
        ...
        @foreach ($questions as $question)
            @php
                ...
                $defaultValue = old('answers.' . $question->id, $existingAnswer ? $existingAnswer->answer_value : null);
            @endphp
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center">
                            <input type="radio" 
                                   name="answers[{{ $question->id }}]" 
                                   value="yes" 
                                   {{ $defaultValue === 'yes' || $defaultValue === '1' ? 'checked' : '' }}
                        ...
                        <label class="flex items-center">
                            <input type="radio" 
                                   name="answers[{{ $question->id }}]" 
                                   value="no" 
                                   {{ $defaultValue === 'no' || $defaultValue === '0' ? 'checked' : '' }}
                                   class="w-4 h-4 text-purple-600 focus:ring-purple-500 border-gray-300">
                        ...
                        @foreach ($question->options as $option)
                            <label class="flex items-center">
                                <input type="radio" 
                                       ...
                                       {{ $defaultValue === $option->option_value ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 focus:ring-purple-500 border-gray-300">
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('client.reviews.show', $review) }}" 
