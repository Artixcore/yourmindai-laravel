...
<div class="mb-4">
    <x-breadcrumb :items="[
        ...
        ['label' => 'Check-in Reviews', 'url' => route('admin.behavior-contingency.checkin-reviews.index')],
        ['label' => $plan->title]
    ]" />
    <h1 class="h3 mb-1 fw-semibold">Review Check-ins</h1>
    <p class="text-muted mb-0">{{ $plan->title }}</p>
</div>
                    ...
                    <div class="modal fade" id="modal-{{ $checkin->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form method="POST" action="{{ route('admin.behavior-contingency.checkins.update', $checkin) }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" name="applied_reward" value="{{ $checkin->applied_reward }}">
                                        </div>
                                        <div class="mb-0">
                                            <label class="form-label">Applied Punishment</label>
                                            <input type="text" class="form-control" name="applied_punishment" value="{{ $checkin->applied_punishment }}">
                                        </div>
                                    </div>
...
<div class="mt-3">
    <a href="{{ route('admin.behavior-contingency.checkin-reviews.index') }}" class="btn btn-outline-secondary">
